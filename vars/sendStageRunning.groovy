...
def call(messages) {
  ...
  for (int i = 0; i < stageNames.size(); i++){
    if ("${stageNames[i]}" == "${env.STAGE_NAME}"){
      ...
      httpRequest consoleLogResponseBody: true, contentType: 'APPLICATION_JSON', httpMode: 'POST', requestBody: payload, url: "${env.SLACK_WEBHOOK_URL}/api/chat.update", validResponseCodes: '200'
    }
  }
